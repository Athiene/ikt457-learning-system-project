...
class BP:
    ...
    def __init__(self, size, cell_node_feature_list, cell_nodes_edge_list, current_winning_path, red_path, move_list,
                 All_Edges, RedAI, BlueAI, red_bp, blue_bp, blue_path, playerColor, enable_print):
    ...
    def get_next_move(self):
        ...
        if len(self.MoveList) < 2:
            ...
            self.conditional_print(f"get_next_move: Current Position being evaluated: {current_position} for {playerColor} ")

            if playerColor == "Red":
                if self.redAI:
                    self.conditional_print("get_next_move: Red is using AI")
                    index = self.get_next_move_with_AI()
                else:
                    self.conditional_print("get_next_move: Red is not using AI")

            if playerColor == "Blue":
                if self.blueAI:
                    self.conditional_print("get_next_move: Blue is using AI")
                    index = self.get_next_move_with_AI()
                else:
                    self.conditional_print("get_next_move: Blue is not using AI")
            return index
    ...
    def get_next_move_with_AI(self):
        ...
        if self.previous_disruption:
            ...
            current_position = self.MoveList[-2]
            self.conditional_print(f"get_next_move_with_AI: No disruption, current_position set to {current_position}")
        playerColor = self.CellNodesFeatureList[current_position]
        self.conditional_print(f"get_next_move_with_AI: Current position {current_position} before detect paths")
        ...
        if path_disruption is not None:
            ...
            self.previous_disruption = False
            self.conditional_print("get_next_move_with_AI: No path disruption detected.")
        self.conditional_print(f"get_next_move_with_AI: {current_position} after path_disruption and before winning path ")
        ...
        self.conditional_print(f"get_next_move_with_AI: Current position before step 1 : {current_position}")
        ...
        self.conditional_print(f"Current position before step 2 : {current_position}")
        self.conditional_print(f"get_next_move_with_AI: Current position before step 2 : {current_position}")
        ...
        self.conditional_print(f"get_next_move_with_AI: STEP 2: Detected wall-adjacent neighbor: {neighbor_with_wall}")
        ...
        self.conditional_print("get_next_move_with_AI: STEP 2/3:  No wall-adjacent neighbors detected, proceeding to bridge detection")
        ...
        if closest_opposite_wall_index:
            self.conditional_print("get_next_move_with_AI: STEP 3:  no bridgesfound , retuning index as none")
            return index
        else:
            index = None
    ...
    def opposite_wall_cloesest_index(self, current_position):
        ...
        if self.Player1:
            ...
            self.conditional_print(f"opposite_wall_cloesest_index: neighbours for {current_position}: {neighbours}")
            ...
            if touching_bot_wall is True:
                ...
                if self.CellNodesFeatureList[lowest_neighbour] == "None":
                    ...
                    self.conditional_print(f"opposite_wall_cloesest_index: chose {index} as move since its closest to top wall ")
                    return index
            ...
            if touching_top_wall is True:
                ...
                if self.CellNodesFeatureList[highest_neighbour] == "None":
                    ...
                    self.conditional_print(f"opposite_wall_cloesest_index: chose {index} as move since its closest to bot wall ")
                    return index
            ...
            for node in self.Current_Winning_Path:
                if node in left_wall_nodes:
                    self.conditional_print(f"opposite_wall_cloesest_index: left_wall_nodes: {left_wall_nodes} and for current path: {self.Current_Winning_Path} the node {node}")
                    touching_left_wall = True

            touching_right_wall = False
            for node in self.Current_Winning_Path:
                if node in right_wall_nodes:
                    self.conditional_print(f"opposite_wall_cloesest_index: right_wall_nodes: {right_wall_nodes} and for current path: {self.Current_Winning_Path} the node {node}")
            ...
            if touching_right_wall is True:
                ...
                if self.CellNodesFeatureList[right_neighbour] == "None":
                    ...
                    self.conditional_print(f"opposite_wall_cloesest_index: chose {index} as move since its closest to left wall ")
                    return index
            ...
            if touching_left_wall is True:
                ...
                if self.CellNodesFeatureList[left_neighbour] == "None":
                    ...
                    self.conditional_print(f"opposite_wall_cloesest_index: chose {index} as move since its closest to right wall ")
                    return index
    ...
    def detect_paths(self):
        ...
        if self.Player1:
            ...
            for i, red_index in enumerate(red_indexes):
                ...
                for j in range(i + 1, len(red_indexes)):
                    ...
                    if next_red_index in edges_for_index_i:
                        ...
                        for path in self.RedPaths:
                            if red_index in path or next_red_index in path:
                                ...
                                path_found = True
                                self.conditional_print(f"detect_paths: Added {red_index} and {next_red_index} to existing path: {path}")
                                break

                        # If no path was found, create a new separate path for these connected nodes
                        if not path_found:
                            self.RedPaths.append([red_index, next_red_index])
                            self.conditional_print(f"detect_paths: Created new path: [{red_index}, {next_red_index}]")
                    ...
                    if len(shared_edges_list) == 2:
                        self.conditional_print(f"detect_paths: Shared edges: {shared_edges_list}")
            ...
            self.conditional_print(self.RedPaths)
            ...
            for i, blue_index in enumerate(blue_indexes):
                ...
                for j in range(i + 1, len(blue_indexes)):
                    ...
                    if next_blue_index in edges_for_index_i:
                        self.conditional_print( f"detect_paths: Bonded path exists between blue index {blue_index} and blue index {blue_index} (neighbors).")
                        path_found = False
                       ...
                       # Search for existing path that includes red_index or next_red_index
                        for path in self.BluePaths:
                            if blue_index in path or next_blue_index in path:
                                ...
                                path_found = True
                                self.conditional_print(f"detect_paths: Added {blue_index} and {next_blue_index} to existing path: {path}")
                                break

                        # If no path was found, create a new separate path for these connected nodes
                        if not path_found:
                            self.BluePaths.append([blue_index, next_blue_index])
                            self.conditional_print(f"detect_paths: Created new path: [{blue_index}, {next_blue_index}]")
            ...
            # Step: Find the path with the longest top-to-bottom coverage
            longest_path = None
            max_horizontal_coverage = 0
            self.Current_Winning_Path = []


            for path in self.BluePaths:
                if path:
                    # Calculate column indices based on row_size
                    column_indices = [index % self.board_size for index in path]
                    min_column = min(column_indices)
                    max_column = max(column_indices)
                    horizontal_coverage = max_column - min_column

                    self.conditional_print(
                        f"detect_paths: Evaluating path {path}: min index = {min_column}, max index = {max_column}, coverage = {horizontal_coverage}")

                    # Update if this path has the longest coverage seen so far
                if horizontal_coverage > max_horizontal_coverage:
                    max_horizontal_coverage = horizontal_coverage
                    longest_path = path
            ...
            if longest_path is not None:
                self.Current_Winning_Path = longest_path
                self.conditional_print(
                    f"\ndetect_paths: The path with the longest top-to-bottom coverage is: {longest_path} with coverage of {max_horizontal_coverage}")
            else:
                self.conditional_print("\ndetect_paths: No valid paths found.")
                self.Current_Winning_Path = []
            ...
            self.conditional_print(self.BluePaths)
    ...
    def winning_path(self):
        ...
        if self.Player1: 
            ...
            if touching_top_wall is True and touching_bot_wall is True:
                self.conditional_print(f"current_winning_path {self.Current_Winning_Path} is touching both the top and bottom walls.")
                ...
                for i in range(len(self.Current_Winning_Path)):
                    for j in range(i + 1, len(self.Current_Winning_Path)):  # Avoid duplicates by starting from i+1
                        ...
                        self.conditional_print(f"current_winning_path: shared_edges_list: {shared_edges_list}")
                        ...
                        if fully_connected:
                            ...
                            if len(shared_edges_list) == 2:
                                if shared_edges_list[0] not in self.all_edges[shared_edges_list[1]]:
                                    self.conditional_print("current_winning_path: This is not a bridge pattern")
                                    continue
                            ...
                            if (self.CellNodesFeatureList[shared_edges_list[0]] == "None" and self.CellNodesFeatureList[
                                ...
                                self.conditional_print(f"current_winning_path: Filled edge {fill_bp_index} between nodes {node_a} and {node_b}.")
            ...
            for node in self.Current_Winning_Path:
                if node in left_wall_nodes:
                    self.conditional_print(f"current_winning_path: left_wall_nodes: {left_wall_nodes} and for current path: {self.Current_Winning_Path} the node {node}")
                    touching_left_wall = True

            touching_right_wall = False
            for node in self.Current_Winning_Path:
                if node in right_wall_nodes:
                    self.conditional_print(f"current_winning_path: right_wall_nodes: {right_wall_nodes} and for current path: {self.Current_Winning_Path} the node {node}")
                    touching_right_wall = True


            # Combined condition to check if the path touches both the top and bottom walls
            if touching_left_wall is True and touching_right_wall is True:
                self.conditional_print(f"current_winning_path {self.Current_Winning_Path} is touching both the right and left walls.")
    ...
    def disrupted_paths(self):
        ...
        if current_player_color == "Red":
            for i in range(len(self.Current_Winning_Path)):
                for j in range(i + 1, len(self.Current_Winning_Path)):  # Avoid duplicates by starting from i+1
                    ...
                    self.conditional_print(f"disrupted_paths: Shared edges list: {shared_edges_list}")
                    self.conditional_print(f"disrupted_paths: Length of Shared edges list: {len(shared_edges_list)}")

                    if len(shared_edges_list) < 1:
                        self.conditional_print(f"disrupted_paths: Self.all_edges[[shared_edges_list[1]]]: Empty")
                        continue
                    ...
                    if occupied_by_self:
                        ...
                        self.conditional_print(f"disrupted_paths: Returning unoccupied cell index {unoccupied_index}.")
        ...
        if current_player_color == "Blue":
            ...
            for i in range(len(self.Current_Winning_Path)):
                ...
                for j in range(i + 1, len(self.Current_Winning_Path)):  # Avoid duplicates by starting from i+1
                    self.conditional_print(f"disrupted_paths -> Current_Winning_Path for blue: {self.Current_Winning_Path}")
    ...
    def switch_position_on_wall_contact(self, current_position):
        ...
        if playerColor == "Red":
            ...
            if current_position < self.board_size or current_position >= self.board_size * (self.board_size - 1):
                self.conditional_print(f"switch_position_on_wall_contact: Current position {current_position} is next to a wall.")
                ...
                for path in self.RedPaths:
                    if current_position in path:
                        ...
                        if len(unique_path_elements) > 1:
                            ...
                            self.conditional_print(f"switch_position_on_wall_contact: Current position {current_position} is the only unique element in the path or isn't in a path, so no movement is needed.")
            ...
            for neighbor in neighbours:
                ...
                if neighbor < self.board_size:
                    if touching_top_wall is False:
                        self.conditional_print(f"switch_position_on_wall_contact: Neighbor {neighbor} is touching the top wall.")
                    ...
                    if touching_bot_wall is False:
                        self.conditional_print(f"switch_position_on_wall_contact: Neighbor {neighbor} is touching the bottom wall.")
                        wall_adjacent_neighbors.append(neighbor)
                    else:
                        self.conditional_print(
        ...
        if playerColor == "Blue":
            if current_position % self.board_size == 0 or current_position % self.board_size == self.board_size - 1:
                self.conditional_print(f"switch_position_on_wall_contact: la Current position {current_position} is next to a left or right wall.")

                # Check if current_position is part of any path in BluePaths
                self.conditional_print(f"switch_position_on_wall_contact: la BluePaths: {self.BluePaths}")
                self.conditional_print(f"switch_position_on_wall_contact: la Blue bridge patterns: {self.Blue_Bp}")

                for path in self.BluePaths:
                    if current_position in path:
                        unique_path_elements = list(set(path))
                        self.conditional_print(f"switch_position_on_wall_contact: la Unique elements in the path containing {current_position} are {unique_path_elements}")
                        ...
                        if len(unique_path_elements) > 1:
                            ...
                            self.conditional_print(f"switch_position_on_wall_contact: lala  Most right index in the path: {most_right}")
                            self.conditional_print(f"switch_position_on_wall_contact: lala Most left index in the path: {most_left}")

                            # Check if current_position is the highest in the path (bottom wall contact), then move to the lowest
                            if current_position == most_right:
                                new_position = most_left
                                self.conditional_print(f"switch_position_on_wall_contact: lala Current position {current_position} is the most right in the path (touching right  wall). Moving to new current position: {new_position}")
                                return new_position

                            # Check if current_position is the lowest in the path (top wall contact), then move to the highest
                            elif current_position == most_left:
                                new_position = most_right
                                self.conditional_print(f"switch_position_on_wall_contact: lala Current position {current_position} is the most left in the path (touching left wall). Moving to new current position: {new_position}")
                                return new_position
            ...
            for neighbor in neighbours:
                ...
                if neighbor % self.board_size == 0:
                    if touching_right_wall is False:
                        self.conditional_print(f"switch_position_on_wall_contact: Neighbor {neighbor} is touching the right wall.")
                        wall_adjacent_neighbors.append(neighbor)
                    else:
                        self.conditional_print(
                    ...
                    if touching_left_wall is False:
                        self.conditional_print(f"switch_position_on_wall_contact: Neighbor {neighbor} is touching the left wall.")
                        wall_adjacent_neighbors.append(neighbor)
                    else:
                        self.conditional_print(
    ...
    def detect_neighbours_is_with_wall(self, current_position):
        ...
        if playerColor == "Red":
            for neighbor in neighbours:
                ...
                if neighbor < self.board_size and self.CellNodesFeatureList[
                    ...
                    self.conditional_print(f"check_if_neighbours_is_with_wall: Neighbor {neighbor} is touching the top wall.")
                    wall_adjacent_neighbors.append(neighbor)
                ...
                # if neigbour is touching bot wall and is empty and the current positions is in a path that is not touching bot wall: append that neibour
                    ...
                    self.conditional_print(f"check_if_neighbours_is_with_wall: Neighbor {neighbor} is touching the bottom wall.")
                    wall_adjacent_neighbors.append(neighbor)
        ...
        if playerColor == "Blue":
            for neighbours in neighbours:
                ...
                if neighbours % self.board_size == 0:
                    self.conditional_print(f"check_if_neighbours_is_with_wall: Neighbor {neighbours} is touching the left wall.")
                ...
                # if neighbours are touching the right wall, append those neighbours indexes in wall_adjacent_neighbours
                    ...
                    self.conditional_print(f"check_if_neighbours_is_with_wall: Neighbor {neighbours} is touching the right wall.")
                    wall_adjacent_neighbors.append(neighbours)
        ...
        if wall_adjacent_neighbors:
            ...
            self.conditional_print(f"check_if_neighbours_is_with_wall: Selected wall-adjacent neighbor: {index}")
        else:
            self.conditional_print(f"check_if_neighbours_is_with_wall: No wall adjacent neighbors found, gonna do evaluate bridge")
    ...
    def detect_bridge(self, index):
        ...
        self.conditional_print(f"detect_bridge: Evaluating bridge patterns for index {index} with color {playerColor}")
    ...
    def evaluate_bridge(self, current_position):
        ...
        if playerColor == "Red":
            ...
            self.conditional_print(f"evaluate_bridge: Evaluate bridge happening from starting index: {current_position}")

            self.detect_bridge(current_position)
            bridge_patterns = self.PossibleBridgesList[current_position]

            if not bridge_patterns:
                self.conditional_print(f"evaluate_bridge: No bridge patterns found in PossibleBridgesList")
            ...
            # Combined condition to check if the path touches both the top and bottom walls
            if touching_top_wall is True and touching_bot_wall is True:
                self.conditional_print(f"evaluate_bridge: {self.Current_Winning_Path} is touching both the top and bottom walls.")

            if bridge_patterns:
                if self.CellNodesFeatureList[current_position] == "Red":
                    self.conditional_print(f"Evaluate bridge happening from {self.CellNodesFeatureList[current_position]}")

                    # Filter out bridge patterns that would lead to top wall positions if the path touches the top wall
                    if touching_top_wall is True:
                        self.conditional_print(f"Bridge_Patter_Lis_TopWall: {bridge_patterns}")
                        self.conditional_print(
                    ...
                    if touching_bot_wall is True:
                        self.conditional_print(f"Bridge_Patter_Lis_BottomWall: {bridge_patterns}")
                        self.conditional_print(
        ...
        if playerColor == "Blue":
            ...
            for node in self.Current_Winning_Path:
                if node in left_wall_nodes:
                    self.conditional_print(f"evaluate_bridge: left_wall_nodes: {left_wall_nodes} and for current path: {self.Current_Winning_Path} the node {node}")
                    touching_left_wall = True

            touching_right_wall = False
            for node in self.Current_Winning_Path:
                if node in right_wall_nodes:
                    self.conditional_print(f"evaluate_bridge: right_wall_nodes: {right_wall_nodes} and for current path: {self.Current_Winning_Path} the node {node}")
                    touching_right_wall = True


            # Combined condition to check if the path touches both the top and bottom walls
            if touching_left_wall is True and touching_right_wall is True:
                self.conditional_print(f"evaluate_bridge: {self.Current_Winning_Path} is touching both the right and left walls.")
            ...
            if bridge_patterns:
                if self.CellNodesFeatureList[current_position] == "Blue":
                    self.conditional_print(f"Evaluate blue bridge happening from {self.CellNodesFeatureList[current_position]}")


                    # Filter out bridge patterns that would lead to top wall positions if the path touches the top wall
                    if touching_left_wall is True:
                        self.conditional_print(f"evaluate_bridge: is left wall: {bridge_patterns}")
                        self.conditional_print(f"evaluate_bridge: hei Path containing {current_position} touches the left wall. Filtering out bridges to top wall positions.")

                        selected_pattern = max(bridge_patterns)

                        self.Blue_Bp.append(selected_pattern)
                        self.conditional_print( f"evaluate_bridge: hei Selected highest index pattern: {selected_pattern} from possible patterns {bridge_patterns}")
                    # Prioritize lowest bridge pattern for upward movement if path touches the bottom wall
                    if touching_right_wall is True:
                        self.conditional_print(f"evaluate_bridge: is right wall:: {bridge_patterns}")
                        self.conditional_print(f"evaluate_bridge: hei Path containing {current_position} touches the right wall. Prioritizing the most right bridge pattern for leftwards movement.")
                        selected_pattern = min(bridge_patterns, key=lambda pattern: pattern % self.board_size)
                        self.conditional_print( f"evaluate_bridge: hei Selected most left index pattern: {selected_pattern} from possible patterns {bridge_patterns}")
                        self.Blue_Bp.append(selected_pattern)
                        ...
                        if selected_pattern is None:
                            ...
                            self.conditional_print( f"evaluate_bridge: Selected closest index: {selected_pattern} from possible patterns {bridge_patterns}")
                        self.conditional_print(f"evaluate_bridge: Blue selected bridge pattern index: {selected_pattern} from possible patterns {bridge_patterns}")
        ...
        self.conditional_print(f"evaluate_bridge: Removed {selected_pattern} from all sublists in PossibleBridgesList")
